...
The [sampling strategy followed in the United Arab Emirates]() from <u>**January 1st, 2022**</u> comprised the sequencing of samples from individuals that belonged to one of the following groups:

1.  All people in ICU/HDU and/or deceased.
2.  All people who entered through a port of entry and had a positive PCR.
3.  The rest of the infected people were sequenced based on a random sample proportionally to the number of positives reported by each lab on a weekly basis.
...
data.lineages %>% filter(Sequenced == 1) %>% 
  ...
  geom_text(aes(label=round(n,2)), position=position_dodge(width=0.9), vjust=-0.25, size=2) +
  scale_y_continuous(labels = percent) +
          xlab("Lineage sequenced") + ylab("# people") +
  ggtitle("Percentage of sequenced records before removing duplicate IDs")
...
```{r }
#| label: duplicate-ids-check
#| tab-cap: Number of records per ID number of the 10 most repeated records in the database.
...
```{r }
#| label: patient-multiple-records
#| tab-cap: Multiple records for the same infection(s) for patient with PseudoID.
...
data.lineages.cleaned.dt %>% filter(Sequenced == 1) %>% 
  ...
  ggplot(aes(x = Lineage, y = n/max(n), fill = Lineage)) +
  # geom_col(aes(fill = Lineage.Short)) + 
  geom_bar(stat = "identity",  color = "black") +
  geom_text(aes(label=round(n/max(n), 2)), position=position_dodge(width=0.9), vjust=-0.25, size=2) +
...
startDate <- "2021-07-01"
 final.data.cleaned %>% 
  ...
  summarise(Number.of.lineages = n()) %>% 
   ungroup() %>%
  ggplot(aes(x = year_month, y = Number.of.lineages/max(Number.of.lineages), fill = Lineage)) +
          # geom_line(lwd = 1) + 
...
<!-- A table summary for variants and reason for sequencing is shown in @tab-table-by-variant: -->

<!-- ```{r} -->
<!-- #| label: table-by-variant -->
<!-- #| tbl-cap: "Summary table of number of cases, hospitalisations ICUs and deaths per variant" -->

<!-- summary.variants <- final.data.cleaned %>%   -->
<!--   # filter(Sequenced == "Sequenced") %>%              -->
<!--           group_by(Reason.for.Sequencing, Lineage) %>%  -->
<!--           summarise(Number.of.cases = n()/max(n()), -->
<!--                     Number.of.hospitalised = sum(Hospitalised == 1)/max(sum(Hospitalised == 1)), -->
<!--                     Number.of.Deaths = sum(Death == 1)/max(sum(Death == 1)),  -->
...
<!-- ```{r comparison-with-estijaba-lineages, fig.cap="Number of cases over time for the different variants reported."} -->
<!-- daily.cases.lineage <- final.data.cleaned %>% group_by(Collection.Date, Lineage) %>%  -->
<!--                                               summarise(nCases = n()/max(n()),  -->
<!--                                                         nCases.smoothed = c(rep(NA, rollmean_days-1), zoo::rollmean(n()/max(n()), k = rollmean_days)))  -->
